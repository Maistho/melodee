...
public class LocalizationServiceTests
{
    ...
    public void Constructor_WithValidDependencies_InitializesSuccessfully()
    {
        ...
        _service.SupportedCultures.Should().HaveCount(20);
    }
    ...
    {
        // Arrange
        var expectedCultures = new[]
        {
            "en-US", "de-DE", "es-ES", "fr-FR", "it-IT", "ja-JP", "pt-BR", "ru-RU", "zh-CN", "ar-SA",
            "nl-NL", "pl-PL", "tr-TR", "id-ID", "ko-KR", "vi-VN", "fa-IR", "uk-UA", "cs-CZ", "sv-SE"
        };
    ...
    {
        // Arrange
        var unsupportedCulture = new CultureInfo("th-TH"); // Thai not supported
        _mockLocalStorage.Setup(x => x.SetItemAsStringAsync(It.IsAny<string>(), It.IsAny<string>()))
            .Returns(Task.CompletedTask);
    ...
    public async Task GetUserCultureAsync_WithUnsupportedStoredCulture_ReturnsDefault()
    {
        // Arrange
        _mockLocalStorage.Setup(x => x.GetItemAsStringAsync("user_preferred_culture"))
            .ReturnsAsync("th-TH"); // Thai not in supported list
