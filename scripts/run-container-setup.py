...
def create_compose_override_for_rootless(project_root: Path) -> bool:
    """
...
# This ensures files created in volumes are owned by the host user
#
...
services:
  melodee.blazor:
    # Run as host user to avoid UID mapping issues in rootless podman
    user: "{uid}:{gid}"
    environment:
...
"""
    ...
    try:
        ...
        print_info("  Files in volumes will be owned by your user")
        return True
    except OSError as e:
...
def main():
    ...
    if is_rootless_podman(runtime):
        print_info("\nDetected rootless podman - fixing volume permissions...")
        if not create_compose_override_for_rootless(project_root):
