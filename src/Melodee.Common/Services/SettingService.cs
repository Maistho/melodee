...
public class SettingService : ServiceBase
{
    ...
    private static IQueryable<Setting> ApplySingleFilter(IQueryable<Setting> query, FilterOperatorInfo filter)
    {
        return filter.Operator switch
        {
            ...
            FilterOperator.Contains => query.Where(s => EF.Functions.ILike(EF.Property<string>(s, filter.PropertyName), $"%{filter.Value}%")),
            FilterOperator.DoesNotContain => query.Where(s => !EF.Functions.ILike(EF.Property<string>(s, filter.PropertyName), $"%{filter.Value}%")),
            FilterOperator.StartsWith => query.Where(s => EF.Functions.ILike(EF.Property<string>(s, filter.PropertyName), $"{filter.Value}%")),
            FilterOperator.EndsWith => query.Where(s => EF.Functions.ILike(EF.Property<string>(s, filter.PropertyName), $"%{filter.Value}")),
