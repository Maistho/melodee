    ...
    configurationFactory)
{
    ...
    /// <summary>
    /// Get artists the user has starred/liked (favorited).
    /// </summary>
    [HttpGet]
    [Route("starred")]
    [ProducesResponseType(typeof(ArtistPagedResponse), StatusCodes.Status200OK)]
    [ProducesResponseType(typeof(ApiError), StatusCodes.Status401Unauthorized)]
    [ProducesResponseType(typeof(ApiError), StatusCodes.Status400BadRequest)]
    public async Task<IActionResult> StarredArtistsAsync(short page = 1, short pageSize = 20, CancellationToken cancellationToken = default)
    {
        if (!ApiRequest.IsAuthorized)
        {
            return ApiUnauthorized();
        ...
        var starredResult = await artistService.ListStarredAsync(new PagedRequest
        {
            Page = validatedPage,
            PageSize = validatedPageSize
        }, user.Id, cancellationToken).ConfigureAwait(false);
        ...
        return Ok(new
        {
            data = starredResult.Data.Select(x => x.ToArtistModel(baseUrl, user.ToUserModel(baseUrl))).ToArray(),
            meta = new
            {
    ...
    /// <summary>
    /// Get artists the user has marked as disliked.
    /// </summary>
    [HttpGet]
    [Route("hated")]
    [ProducesResponseType(typeof(ArtistPagedResponse), StatusCodes.Status200OK)]
    [ProducesResponseType(typeof(ApiError), StatusCodes.Status401Unauthorized)]
    [ProducesResponseType(typeof(ApiError), StatusCodes.Status400BadRequest)]
    public async Task<IActionResult> HatedArtistsAsync(short page = 1, short pageSize = 20, CancellationToken cancellationToken = default)
    {
        if (!ApiRequest.IsAuthorized)
        {
            return ApiUnauthorized();
        ...
        var hatedResult = await artistService.ListHatedAsync(new PagedRequest
        {
            Page = validatedPage,
            PageSize = validatedPageSize
        }, user.Id, cancellationToken).ConfigureAwait(false);
        ...
        return Ok(new
        {
            data = hatedResult.Data.Select(x => x.ToArtistModel(baseUrl, user.ToUserModel(baseUrl))).ToArray(),
            meta = new
            {
    ...
    /// <summary>
    /// Get artists the user has rated 4+ stars.
    /// </summary>
    [HttpGet]
    [Route("top-rated")]
    [ProducesResponseType(typeof(ArtistPagedResponse), StatusCodes.Status200OK)]
    [ProducesResponseType(typeof(ApiError), StatusCodes.Status401Unauthorized)]
    [ProducesResponseType(typeof(ApiError), StatusCodes.Status400BadRequest)]
    public async Task<IActionResult> TopRatedArtistsAsync(short page = 1, short pageSize = 20, CancellationToken cancellationToken = default)
    {
        if (!ApiRequest.IsAuthorized)
        {
            return ApiUnauthorized();
        ...
        var topRatedResult = await artistService.ListTopRatedAsync(new PagedRequest
        {
            Page = validatedPage,
            PageSize = validatedPageSize
        ...
        return Ok(new
        {
            data = topRatedResult.Data.Select(x => x.ToArtistModel(baseUrl, user.ToUserModel(baseUrl))).ToArray(),
            meta = new
            {
