...
(function () {
    ...
    function initializeReconnectionHandler() {
        // Ensure Blazor and defaultReconnectionHandler exist before configuring
        if (!window.Blazor || !window.Blazor.defaultReconnectionHandler) {
            console.warn('Blazor.defaultReconnectionHandler not yet available, retrying...');
            setTimeout(initializeReconnectionHandler, 100);
            return;
        }

        // Enhanced Blazor reconnection configuration
        Blazor.defaultReconnectionHandler._reconnectCallback = async function () {
            reconnectAttempts++;
        ...
        document.addEventListener('visibilitychange', () => {
            if (document.hidden) {
                ...
                if (reconnectModal && Blazor._internal?.navigationManager?.isConnected === false) {
    ...
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', initializeReconnectionHandler);
    } else {
