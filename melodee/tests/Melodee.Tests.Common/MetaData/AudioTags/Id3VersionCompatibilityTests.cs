...
public class Id3VersionCompatibilityTests
{
    ...
    public async Task Should_Read_ID3v1_1_Tags()
    {
        ...
        try
        {
            ...
            var commentPart = metadata.Comment.Substring(0, Math.Min(metadata.Comment.Length, 28));
    ...
    public async Task Should_Read_ID3v2_4_Tags()
    {
        ...
        try
        {
            ...
            var comment = SafeParser.ToString(tags.Tags[MetaTagIdentifier.Comment]);
    ...
    public async Task Should_Prioritize_ID3v2_Over_ID3v1_When_Both_Present()
    {
        ...
        using (var fileStream = new FileStream(filePath, FileMode.Open, FileAccess.ReadWrite))
        {
            ...
            using (var writer = new BinaryWriter(fileStream, Encoding.ASCII, true))
            {
                ...
                var titleBytes = Encoding.ASCII.GetBytes(v1Metadata.Title.PadRight(30).Substring(0, 30));
                writer.Write(titleBytes);

                // Artist (30 bytes)
                var artistBytes = Encoding.ASCII.GetBytes(v1Metadata.Artist.PadRight(30).Substring(0, 30));
                writer.Write(artistBytes);

                // Album (30 bytes)
                var albumBytes = Encoding.ASCII.GetBytes(v1Metadata.Album.PadRight(30).Substring(0, 30));
                writer.Write(albumBytes);

                // Year (4 bytes)
                var yearBytes = Encoding.ASCII.GetBytes(v1Metadata.RecordingYear.ToString().PadRight(4).Substring(0, 4));
                writer.Write(yearBytes);

                // Comment (28 bytes for ID3v1.1)
                var commentBytes = Encoding.ASCII.GetBytes(v1Metadata.Comment.PadRight(28).Substring(0, 28));
    ...
    public async Task Should_Handle_TCON_Genre_With_Parenthesized_Number()
    {
        ...
        try
        {
            ...
            var genreValue = SafeParser.ToString(tags.Tags[MetaTagIdentifier.Genre]);
    ...
    public async Task Should_Handle_Large_Text_Frames_Across_Versions()
    {
        ...
        var longText = new string('A', 5000); // 5000 'A' characters
    ...
    public async Task Should_Read_APIC_Image_Frame_ID3v2_4()
    {
        ...
        var fakeImage = new byte[] { 0x89, 0x50, 0x4E, 0x47, 0x0D, 0x0A, 0x1A, 0x0A, 0x00, 0x01, 0x02, 0x03 };
        var metadata = new BlankMusicFileGenerator.MusicMetadata
        {
            Title = "Image Test",
            AlbumArt = fakeImage
    ...
    public async Task Should_Read_Multiple_Values_In_Text_Frame_ID3v2_4()
    {
        ...
        {
            var tags = await AudioTagManager.ReadAllTagsAsync(filePath, CancellationToken.None);
            var artist = SafeParser.ToString(tags.Tags[MetaTagIdentifier.Artist]);
