...
public class HmacTokenService
{
    ...
    public bool ValidateToken(string data, string token)
    {
        ...
        var expectedToken = GenerateToken(data);
    ...
    {
        var timestamp = DateTimeOffset.UtcNow.AddDays(expirationDays).ToUnixTimeSeconds().ToString();
        var dataWithTimestamp = $"{data}|{timestamp}";
        var token = GenerateToken(dataWithTimestamp);
    ...
    public bool TryValidateTimedToken(string tokenData, out string data, out DateTimeOffset expiration)
    {
        ...
        if (!long.TryParse(timestamp, out var expirationTime))
        {
            return false;
        }
